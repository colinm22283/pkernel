...
#define PDT64_SET_ADDRESS(pdt_entry, _address) do { (pdt_entry).address = ((uint64_t) _address) >> 12; } while (0)
#define PDT64_GET_ADDRESS(pdt_entry) ((uint64_t)(((uint64_t) (pdt_entry).address) << 12))
...
static inline pdt64_entry_t * pdt64_map_address(pdt64_t * pdt, uint64_t pt, void * virtual_address) {
    PDT64_SET_ADDRESS((*pdt)[((intptr_t) virtual_address >> 21) & 0x1FF], pt);
    return &(*pdt)[((intptr_t) virtual_address >> 21) & 0x1FF];
}
